...
#   :8000  chat        — vLLM · Qwen2.5 (Writer + Fact-checker)
#   :8001  embeddings  — sentence-transformers · bge-m3 (Researcher / retrieval)
...
#
# Usage:  export TOKEN=your-secret ; ./amd_stack.sh
set -euo pipefail
...
# 1) Chat (Qwen2.5-7B) — capped so the GPU is shared with embeddings + image.
docker rm -f factor-vllm 2>/dev/null || true
docker run -d --name factor-vllm --restart unless-stopped $GPU --shm-size 16g -p 8000:8000 $VOL "$IMG" \
  bash -lc "vllm serve Qwen/Qwen2.5-7B-Instruct --host 0.0.0.0 --port 8000 \
    --served-model-name qwen2.5-7b-instruct --max-model-len 8192 \
    --gpu-memory-utilization 0.40 --api-key ${TOKEN}"

# 2) Embeddings (bge-m3 via sentence-transformers — vLLM ROCm lacks encoder attention).
docker rm -f factor-embed 2>/dev/null || true
docker run -d --name factor-embed --restart unless-stopped $GPU --shm-size 8g -p 8001:8000 $VOL \
  -e EMBED_API_KEY=${TOKEN} -v "$(dirname "$0")/embed_server.py":/app/embed_server.py "$IMG" \
  bash -lc "pip install -q --no-cache-dir sentence-transformers; python /app/embed_server.py"
